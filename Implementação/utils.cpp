...
int get_int_cin(const char* message, int size, int min) {
	...
	do {
		...
		if (!(cin >> numero)) {
			...
			if (numero >= min && numero < size)
				...
				cout << "Número fora do intervalo!" << endl << endl; //Senão informasse o utilizador e pede-se novamente o nº
...
bool get_yes_no(const char* message, const char* message_erro)
{
...
bool printMenu(string & title, const vector<vector<string> > & columns, const vector<string> & colNames, bool menu, bool reverse, bool hide_index, bool hide_colnames) {
